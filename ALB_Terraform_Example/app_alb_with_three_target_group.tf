...
#==========================================================================
  ...
  target_groups = [
    ...
    #--------------------------------------------------------------------------------------------------
    ## UAM APPLICATION TARGET GROUP --- target_group_index = 1
    {
      name             = join("-", [local.org, local.csp, local.region, local.vpcname, "uam-tg"])
      backend_protocol = "HTTPS"
      backend_port     = 443
      target_type      = "instance"
      protocol_version = "HTTP1"
      ...
      targets = {
        target1 = {
          target_id = var.uam_app_instances[0]
          port = 443
        },
        target2 = {
          target_id = var.uam_app_instances[1]
          port = 443
        },
        target3 = {
          target_id = var.uam_app_instances[2]
          port = 443
        },
      }
      health_check = {
      ...
      tags = merge( { application = "uam-app" }, 
        var.additional_tags
      )
     },
     #--------------------------------------------------------------------------------------------------
     ## UAM AUTH APPLICATION TARGET GROUP -  target_group_index = 2
      {
      name             = join("-", [local.org, local.csp, local.region, local.vpcname, "uauth-tg"])
      backend_protocol = "HTTPS"
      backend_port     = 443
      target_type      = "instance"
      protocol_version = "HTTP1"
      ...
      targets = {
        target1 = {
          target_id = var.uam_auth_app_instances[0]
          port = 443
        },
        target2 = {
          target_id = var.uam_auth_app_instances[1]
          port = 443
        },
        target3 = {
          target_id = var.uam_auth_app_instances[2]
          port = 443
        },
      }
      health_check = {
       ...
       matcher             = "200-399"
    	...
    	tags = merge( { application = "uam-auth-app" }, 
        var.additional_tags
      )
     }
     #--------------------------------------------------------------------------------------------------     
